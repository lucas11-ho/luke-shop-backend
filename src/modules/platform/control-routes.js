...
import { normalizeExperienceConfig, STATUS_VISUAL_PACKS, STATUS_VISUAL_ICON_KEYS, STATUS_VISUAL_STATUSES } from '../customer-experience/service.js';
...
export async function platformControlRoutes(app){
  ...
  app.get('/v1/platform/status-visual-packs',{preHandler:auth},async()=>({data:{packs:(await app.db.query("SELECT key,name,business_type,status,icons,settings,updated_at FROM platform_status_visual_packs ORDER BY business_type,name")).rows,allowed_icons:STATUS_VISUAL_ICON_KEYS,statuses:STATUS_VISUAL_STATUSES}}));


  app.patch('/v1/platform/status-visual-packs/:packKey',{preHandler:[app.requirePlatformAuth,app.requirePlatformOwner],schema:{body:{type:'object',additionalProperties:false,minProperties:1,properties:{name:{type:'string',minLength:2,maxLength:120},status:{type:'string',enum:['ACTIVE','INACTIVE']},icons:{type:'object',additionalProperties:{type:'string',enum:STATUS_VISUAL_ICON_KEYS}},settings:{type:'object'}}}}},async request=>app.db.transaction(async client=>{const key=String(request.params.packKey||'').trim().toUpperCase();if(key==='AUTO'||!STATUS_VISUAL_PACKS.includes(key))throw errors.notFound('STATUS_VISUAL_PACK_NOT_FOUND','Status visual pack not found');const found=await client.query('SELECT * FROM platform_status_visual_packs WHERE key=$1 FOR UPDATE',[key]);if(!found.rowCount)throw errors.notFound('STATUS_VISUAL_PACK_NOT_FOUND','Status visual pack not found');const row=found.rows[0];if(request.body.icons){for(const status of Object.keys(request.body.icons)){if(!STATUS_VISUAL_STATUSES.includes(status))throw errors.badRequest('STATUS_VISUAL_STATUS_INVALID',`Unsupported fulfillment status icon mapping: ${status}`)}}await client.query('UPDATE platform_status_visual_packs SET name=$1,status=$2,icons=$3::jsonb,settings=$4::jsonb,updated_at=now() WHERE key=$5',[request.body.name??row.name,request.body.status??row.status,JSON.stringify(request.body.icons??row.icons),JSON.stringify(request.body.settings??row.settings),key]);await writePlatformAudit(client,{actorId:request.platformAuth.actorId,action:'status_visual_pack.update',targetType:'status_visual_pack',metadata:{key,changed_fields:Object.keys(request.body)},requestIp:request.ip,requestId:request.id});return {data:{pack:(await client.query('SELECT key,name,business_type,status,icons,settings,updated_at FROM platform_status_visual_packs WHERE key=$1',[key])).rows[0]}};}));
