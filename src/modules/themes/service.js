...
export const PLATFORM_NAV_ICON_TOKEN_PREFIX='platform:';
...
const APP_SET=new Set(THEME_APPS),HEX=/^#[0-9a-fA-F]{6}$/,KEY=/^[A-Z][A-Z0-9_]{1,79}$/,VERSION=/^[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?$/,VARIANT=/^[a-z][a-z0-9_-]{0,63}$/,PACK_KEY=/^[A-Z][A-Z0-9_]{1,79}$/,PLATFORM_ICON_KEY=/^[A-Z0-9][A-Z0-9._-]{2,79}$/;
...
function normalizePlatformNavigationIconToken(value){const raw=String(value||'').trim();if(!raw.toLowerCase().startsWith(PLATFORM_NAV_ICON_TOKEN_PREFIX))return'';const key=raw.slice(PLATFORM_NAV_ICON_TOKEN_PREFIX.length).trim().toUpperCase();return PLATFORM_ICON_KEY.test(key)?`${PLATFORM_NAV_ICON_TOKEN_PREFIX}${key}`:'';}
export function platformIconKeyFromThemeToken(value){const token=normalizePlatformNavigationIconToken(value);return token?token.slice(PLATFORM_NAV_ICON_TOKEN_PREFIX.length):null;}
function normalizeNavigationIconValue(value){return normalizePlatformNavigationIconToken(value)||safeIdentifier(value);}

export function normalizeThemeNavigationIcons(raw={}){if(!raw||typeof raw!=='object'||Array.isArray(raw))return{};const out={};for(const slot of CUSTOMER_NAV_SLOTS){const icon=normalizeNavigationIconValue(raw[slot]);if(icon)out[slot]=icon;}return out;}
export function validateThemeNavigationIcons(theme,raw={}){const overrides=normalizeThemeNavigationIcons(raw);if(!Object.keys(overrides).length)return{};if(!theme)throw errors.badRequest('THEME_ICON_THEME_REQUIRED','Choose a Customer Web theme before changing navigation icons');const icons=theme.manifest?.icons||{},pack=normalizeIconPack(icons.pack),rendererAllowed=THEME_ICON_PACKS[pack]||[],packageAllowed=normalizeIconAllowList(pack,icons.allowed);for(const[slot,icon]of Object.entries(overrides)){if(!CUSTOMER_NAV_SLOTS.includes(slot))throw errors.badRequest('THEME_NAV_ICON_NOT_ALLOWED',`Navigation icon ${slot}:${icon} is not allowed by the selected theme package`);const platformKey=platformIconKeyFromThemeToken(icon);if(platformKey){if(pack!=='PHOSPHOR_NAV'||icons.allow_custom_images!==true)throw errors.badRequest('THEME_NAV_ICON_NOT_ALLOWED',`Custom navigation icon ${slot}:${platformKey} is not enabled by the selected theme package`);continue;}if(!rendererAllowed.includes(icon)||!packageAllowed.includes(icon))throw errors.badRequest('THEME_NAV_ICON_NOT_ALLOWED',`Navigation icon ${slot}:${icon} is not allowed by the selected theme package`);}return overrides;}
...
export function normalizeThemeComponentOverrides(raw={}){if(!raw||typeof raw!=='object'||Array.isArray(raw))return{};const out={};for(const[key,value]of Object.entries(raw).slice(0,64)){const k=safeIdentifier(key),v=NAV_OVERRIDE_TO_SLOT[k]?normalizeNavigationIconValue(value):safeIdentifier(value);if(k&&v)out[k]=v;}return out;}
...
export function validateThemeComponentOverrides(theme,app,raw={}){
  ...
  for(const[key,variant]of Object.entries(overrides)){
    ...
    if(NAV_OVERRIDE_TO_SLOT[key]){
      const platformKey=platformIconKeyFromThemeToken(variant);
      if(platformKey){if(normalizedApp!=='CUSTOMER_WEB'||iconPack!=='PHOSPHOR_NAV'||icons.allow_custom_images!==true)throw errors.badRequest('THEME_NAV_ICON_NOT_ALLOWED',`Custom navigation icon ${key}:${platformKey} is not enabled by the selected theme package`);continue;}
