...
export async function merchantCatalogRoutes(app) {
  ...
  app.patch('/v1/merchant/products/:productId/modifier-groups/:groupId', {
    preHandler:[app.requireMerchantAuth,app.requirePermission(PERMISSIONS.CATALOG_WRITE)],
    schema:{body:{type:'object',additionalProperties:false,minProperties:1,properties:{
      name:{type:'string',minLength:1,maxLength:160},required:{type:'boolean'},min_selections:{type:'integer',minimum:0,maximum:100},max_selections:{type:'integer',minimum:1,maximum:100},sort_order:{type:'integer',minimum:-100000,maximum:100000},status:{type:'string',enum:['ACTIVE','INACTIVE']}
    }}},
    ...
    const product=await resolveProduct(client,request.auth.tenantId,store.id,request.params.productId);const found=await client.query('SELECT * FROM product_modifier_groups WHERE tenant_id=$1 AND store_id=$2 AND product_id=$3 AND public_id=$4 FOR UPDATE',[request.auth.tenantId,store.id,product.id,request.params.groupId]);if(!found.rowCount)throw errors.notFound('MODIFIER_GROUP_NOT_FOUND','Modifier group not found');const cur=found.rows[0],b=request.body||{};const required=b.required??cur.required;const min=b.min_selections??cur.min_selections,max=b.max_selections??cur.max_selections;if(max<min||(required&&min<1))throw errors.badRequest('MODIFIER_SELECTION_RULE_INVALID','Modifier selection limits are invalid');const updated=await client.query(`UPDATE product_modifier_groups SET name=$1,required=$2,min_selections=$3,max_selections=$4,sort_order=$5,status=$6,updated_at=now() WHERE id=$7 RETURNING public_id,name,required,min_selections,max_selections,sort_order,status,updated_at`,[b.name?.trim()??cur.name,required,min,max,b.sort_order??cur.sort_order,b.status??cur.status,cur.id]);
    if((b.status??cur.status)==='INACTIVE')await client.query("UPDATE product_modifier_options SET status='INACTIVE',updated_at=now() WHERE tenant_id=$1 AND store_id=$2 AND product_id=$3 AND group_id=$4",[request.auth.tenantId,store.id,product.id,cur.id]);
    await writeAudit(client,{tenantId:request.auth.tenantId,actorType:'MERCHANT',actorId:request.auth.actorId,action:'catalog.modifier_group.update',targetType:'modifier_group',targetId:cur.id,metadata:{product_id:product.public_id,changed_fields:Object.keys(b)},requestIp:request.ip,requestId:request.id});return updated.rows[0];});return{data:{modifier_group:row}};});

  app.delete('/v1/merchant/products/:productId/modifier-groups/:groupId', {preHandler:[app.requireMerchantAuth,app.requirePermission(PERMISSIONS.CATALOG_WRITE)]}, async(request)=>{
    const store=await resolveStore(app.db,request.auth.tenantId,storeHeader(request),{requireActive:false});await app.db.transaction(async(client)=>{const product=await resolveProduct(client,request.auth.tenantId,store.id,request.params.productId);const found=await client.query("UPDATE product_modifier_groups SET status='INACTIVE',updated_at=now() WHERE tenant_id=$1 AND store_id=$2 AND product_id=$3 AND public_id=$4 RETURNING id",[request.auth.tenantId,store.id,product.id,request.params.groupId]);if(!found.rowCount)throw errors.notFound('MODIFIER_GROUP_NOT_FOUND','Modifier group not found');await client.query("UPDATE product_modifier_options SET status='INACTIVE',updated_at=now() WHERE tenant_id=$1 AND store_id=$2 AND product_id=$3 AND group_id=$4",[request.auth.tenantId,store.id,product.id,found.rows[0].id]);await writeAudit(client,{tenantId:request.auth.tenantId,actorType:'MERCHANT',actorId:request.auth.actorId,action:'catalog.modifier_group.deactivate',targetType:'modifier_group',targetId:found.rows[0].id,metadata:{product_id:product.public_id},requestIp:request.ip,requestId:request.id});});return{data:{deactivated:true}};
  });

  app.patch('/v1/merchant/products/:productId/modifier-groups/:groupId/options/:optionId', {
    preHandler:[app.requireMerchantAuth,app.requirePermission(PERMISSIONS.CATALOG_WRITE)],
    schema:{body:{type:'object',additionalProperties:false,minProperties:1,properties:{name:{type:'string',minLength:1,maxLength:160},price_delta:{anyOf:[{type:'number'},{type:'string',pattern:'^-?\\d+(?:\\.\\d{1,4})?$'}]},sort_order:{type:'integer',minimum:-100000,maximum:100000},status:{type:'string',enum:['ACTIVE','INACTIVE']}}}},
  },async(request)=>{const store=await resolveStore(app.db,request.auth.tenantId,storeHeader(request),{requireActive:false});const row=await app.db.transaction(async(client)=>{const product=await resolveProduct(client,request.auth.tenantId,store.id,request.params.productId);const group=await client.query('SELECT id FROM product_modifier_groups WHERE tenant_id=$1 AND store_id=$2 AND product_id=$3 AND public_id=$4',[request.auth.tenantId,store.id,product.id,request.params.groupId]);if(!group.rowCount)throw errors.notFound('MODIFIER_GROUP_NOT_FOUND','Modifier group not found');const found=await client.query('SELECT * FROM product_modifier_options WHERE tenant_id=$1 AND store_id=$2 AND product_id=$3 AND group_id=$4 AND public_id=$5 FOR UPDATE',[request.auth.tenantId,store.id,product.id,group.rows[0].id,request.params.optionId]);if(!found.rowCount)throw errors.notFound('MODIFIER_OPTION_NOT_FOUND','Modifier option not found');const cur=found.rows[0],b=request.body||{};const updated=await client.query(`UPDATE product_modifier_options SET name=$1,price_delta=$2,sort_order=$3,status=$4,updated_at=now() WHERE id=$5 RETURNING public_id,name,price_delta,sort_order,status,updated_at`,[b.name?.trim()??cur.name,b.price_delta??cur.price_delta,b.sort_order??cur.sort_order,b.status??cur.status,cur.id]);await writeAudit(client,{tenantId:request.auth.tenantId,actorType:'MERCHANT',actorId:request.auth.actorId,action:'catalog.modifier_option.update',targetType:'modifier_option',targetId:cur.id,metadata:{product_id:product.public_id,group_id:request.params.groupId,changed_fields:Object.keys(b)},requestIp:request.ip,requestId:request.id});return updated.rows[0];});return{data:{modifier_option:row}};});

  app.delete('/v1/merchant/products/:productId/modifier-groups/:groupId/options/:optionId', {preHandler:[app.requireMerchantAuth,app.requirePermission(PERMISSIONS.CATALOG_WRITE)]}, async(request)=>{
    const store=await resolveStore(app.db,request.auth.tenantId,storeHeader(request),{requireActive:false});await app.db.transaction(async(client)=>{const product=await resolveProduct(client,request.auth.tenantId,store.id,request.params.productId);const group=await client.query('SELECT id FROM product_modifier_groups WHERE tenant_id=$1 AND store_id=$2 AND product_id=$3 AND public_id=$4',[request.auth.tenantId,store.id,product.id,request.params.groupId]);if(!group.rowCount)throw errors.notFound('MODIFIER_GROUP_NOT_FOUND','Modifier group not found');const found=await client.query("UPDATE product_modifier_options SET status='INACTIVE',updated_at=now() WHERE tenant_id=$1 AND store_id=$2 AND product_id=$3 AND group_id=$4 AND public_id=$5 RETURNING id",[request.auth.tenantId,store.id,product.id,group.rows[0].id,request.params.optionId]);if(!found.rowCount)throw errors.notFound('MODIFIER_OPTION_NOT_FOUND','Modifier option not found');await writeAudit(client,{tenantId:request.auth.tenantId,actorType:'MERCHANT',actorId:request.auth.actorId,action:'catalog.modifier_option.deactivate',targetType:'modifier_option',targetId:found.rows[0].id,metadata:{product_id:product.public_id,group_id:request.params.groupId},requestIp:request.ip,requestId:request.id});});return{data:{deactivated:true}};
  });
