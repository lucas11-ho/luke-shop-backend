...
import { allowedMime, deleteAsset, extensionForMime, hasExpectedSignature, mediaTypeForMime, safeOriginalFilename, storagePublicUrl, writeAsset } from '../assets/storage.js';
import { allocateCustomerCode, assertCustomerTurnstile, createCustomerSession, customerIdentitySettings, effectiveAuthOptions, findOrCreateProviderCustomer, googleEmailMayAutoLink, issueTelegramNonce, newOtp, normalizePhone, otpHash, publicCustomer, verifyGoogleCredential, verifyTelegramIdToken, verifyTelegramNonce, verifyTelegramPayload } from './customer-identity.js';
...
import { customerMapConfig, reverseGeocode } from '../delivery/geocoding.js';
...
export async function customerAuthRoutes(app) {
  app.post('/v1/customer/auth/register', {
    ...
    if (authOptions.turnstile.signup_required) await assertCustomerTurnstile(app.config, request.body.turnstile_token, request, 'signup');
  ...
  app.get('/v1/customer/auth/telegram/nonce',{preHandler:[app.requireTenant],config:{rateLimit:{max:30,timeWindow:'1 minute'}}},async request=>{const settings=await customerIdentitySettings(app.db,request.tenant.id),opts=effectiveAuthOptions(settings,app.config);if(!opts.methods.telegram.enabled||opts.methods.telegram.mode!=='OIDC_LIBRARY')throw errors.unavailable('TELEGRAM_LOGIN_NOT_CONFIGURED','Telegram modern login is not configured for this store');return {data:{nonce:issueTelegramNonce(app.config,request.tenant.id),expires_in:600}};});

  app.post('/v1/customer/auth/google',{preHandler:[app.requireTenant],config:{rateLimit:{max:20,timeWindow:'1 minute'}},schema:{body:{type:'object',additionalProperties:false,required:['credential'],properties:{credential:{type:'string',minLength:20,maxLength:10000},turnstile_token:{type:'string',minLength:1,maxLength:2048}}}}},async request=>{const settings=await customerIdentitySettings(app.db,request.tenant.id),opts=effectiveAuthOptions(settings,app.config);if(!opts.methods.google.enabled)throw errors.forbidden('GOOGLE_LOGIN_DISABLED','Google login is not enabled for this store');if(opts.turnstile.social_required)await assertCustomerTurnstile(app.config,request.body.turnstile_token,request,'social');const profile=await verifyGoogleCredential(app.config,request.body.credential);return app.db.transaction(async client=>{const row=await findOrCreateProviderCustomer(app,client,{tenantId:request.tenant.id,provider:'GOOGLE',subject:String(profile.sub),displayName:String(profile.name||profile.email||'Customer'),email:String(profile.email),avatarUrl:profile.picture?String(profile.picture):null,metadata:{email_verified:true,hosted_domain:profile.hd||null},allowEmailAutoLink:googleEmailMayAutoLink(profile),request});if(row.status!=='ACTIVE')throw errors.forbidden('CUSTOMER_NOT_ACTIVE','Customer account is not active');const tokens=await createCustomerSession(app,client,request.tenant.id,row.id,request);await client.query('UPDATE customers SET last_login_at=now(),updated_at=now() WHERE id=$1',[row.id]);return {data:{customer:publicCustomer(row),tokens}};});});

  app.post('/v1/customer/auth/telegram',{preHandler:[app.requireTenant],config:{rateLimit:{max:20,timeWindow:'1 minute'}},schema:{body:{type:'object',additionalProperties:true,properties:{id_token:{type:'string',minLength:20,maxLength:12000},nonce:{type:'string',minLength:20,maxLength:2048},turnstile_token:{type:'string',minLength:1,maxLength:2048},id:{},auth_date:{},hash:{type:'string'},first_name:{type:'string'},last_name:{type:'string'},username:{type:'string'},photo_url:{type:'string'}}}}},async request=>{const settings=await customerIdentitySettings(app.db,request.tenant.id),opts=effectiveAuthOptions(settings,app.config);if(!opts.methods.telegram.enabled)throw errors.forbidden('TELEGRAM_LOGIN_DISABLED','Telegram login is not enabled for this store');if(opts.turnstile.social_required)await assertCustomerTurnstile(app.config,request.body.turnstile_token,request,'social');let subject,displayName,avatarUrl,metadata;if(request.body.id_token){if(opts.methods.telegram.mode!=='OIDC_LIBRARY')throw errors.badRequest('TELEGRAM_LOGIN_MODE_INVALID','Telegram modern login is not enabled');verifyTelegramNonce(app.config,request.body.nonce,request.tenant.id);const t=await verifyTelegramIdToken(app.config,request.body.id_token,request.body.nonce);subject=String(t.sub);displayName=String(t.name||t.preferred_username||'Telegram customer');avatarUrl=t.picture?String(t.picture):null;metadata={username:t.preferred_username||null,oidc:true};}else{const t=verifyTelegramPayload(app.config,request.body);subject=String(t.id);displayName=[t.first_name,t.last_name].filter(Boolean).join(' ')||t.username||'Telegram customer';avatarUrl=t.photo_url||null;metadata={username:t.username||null,legacy:true};}return app.db.transaction(async client=>{const row=await findOrCreateProviderCustomer(app,client,{tenantId:request.tenant.id,provider:'TELEGRAM',subject,displayName,avatarUrl,metadata,request});if(row.status!=='ACTIVE')throw errors.forbidden('CUSTOMER_NOT_ACTIVE','Customer account is not active');const tokens=await createCustomerSession(app,client,request.tenant.id,row.id,request);await client.query('UPDATE customers SET last_login_at=now(),updated_at=now() WHERE id=$1',[row.id]);return {data:{customer:publicCustomer(row),tokens}};});});
  ...
  app.post('/v1/customer/me/auth-identities/telegram',{preHandler:[app.requireCustomerAuth],config:{rateLimit:{max:10,timeWindow:'1 minute'}},schema:{body:{type:'object',additionalProperties:true,properties:{id_token:{type:'string',minLength:20,maxLength:12000},nonce:{type:'string',minLength:20,maxLength:2048},id:{},auth_date:{},hash:{type:'string'},first_name:{type:'string'},last_name:{type:'string'},username:{type:'string'},photo_url:{type:'string'}}}}},async request=>app.db.transaction(async client=>{const settings=await customerIdentitySettings(client,request.auth.tenantId),opts=effectiveAuthOptions(settings,app.config);if(!opts.methods.telegram.enabled)throw errors.forbidden('TELEGRAM_LOGIN_DISABLED','Telegram login is not enabled');let subject,metadata;if(request.body.id_token){if(opts.methods.telegram.mode!=='OIDC_LIBRARY')throw errors.badRequest('TELEGRAM_LOGIN_MODE_INVALID','Telegram modern login is not enabled');verifyTelegramNonce(app.config,request.body.nonce,request.auth.tenantId);const t=await verifyTelegramIdToken(app.config,request.body.id_token,request.body.nonce);subject=String(t.sub);metadata={username:t.preferred_username||null,oidc:true};}else{const t=verifyTelegramPayload(app.config,request.body);subject=String(t.id);metadata={username:t.username||null,legacy:true};}const occupied=await client.query(`SELECT customer_id FROM customer_auth_identities WHERE tenant_id=$1 AND provider='TELEGRAM' AND provider_subject=$2`,[request.auth.tenantId,subject]);if(occupied.rowCount&&occupied.rows[0].customer_id!==request.auth.actorId)throw errors.conflict('LOGIN_IDENTITY_IN_USE','This Telegram account is already connected to another customer');await client.query(`INSERT INTO customer_auth_identities(id,public_id,tenant_id,customer_id,provider,provider_subject,metadata,verified_at) VALUES($1,$2,$3,$4,'TELEGRAM',$5,$6::jsonb,now()) ON CONFLICT(tenant_id,customer_id,provider) DO UPDATE SET provider_subject=EXCLUDED.provider_subject,metadata=EXCLUDED.metadata,verified_at=now(),updated_at=now()`,[uuid(),publicId('cid'),request.auth.tenantId,request.auth.actorId,subject,JSON.stringify(metadata)]);await writeAudit(client,{tenantId:request.auth.tenantId,actorType:'CUSTOMER',actorId:request.auth.actorId,action:'customer.login_identity.link',targetType:'customer',targetId:request.auth.actorId,metadata:{provider:'TELEGRAM'},requestIp:request.ip,requestId:request.id});return {data:{linked:true,provider:'TELEGRAM'}};}));
