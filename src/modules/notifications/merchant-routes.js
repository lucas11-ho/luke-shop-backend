...
import { staffPushRoutes } from './push-routes.js';
import { startStaffPushWorker } from './push-service.js';
const storeHeader=r=>r.headers['x-store-id']||null;

export async function merchantNotificationRoutes(app){
  const stopPushWorker=startStaffPushWorker(app);
  app.addHook('onClose',async()=>stopPushWorker());
  await staffPushRoutes(app);
