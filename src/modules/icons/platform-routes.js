...
export async function platformIconRoutes(app){
  ...
  app.post('/v1/platform/icons/:iconKey/retire',{preHandler:[app.requirePlatformAuth,app.requirePlatformOwner]},async request=>app.db.transaction(async client=>{
    ...
    if(row.status!=='RETIRED'){
      ...
      await writePlatformAudit(client,{actorId:request.platformAuth.actorId,action:'icon.library.retire',targetType:'platform_icon',targetId:row.id,metadata:{key:row.key,source_type:row.source_type},requestIp:request.ip,requestId:request.id});
    }
  ...
  app.delete('/v1/platform/icons/:iconKey',{preHandler:[app.requirePlatformAuth,app.requirePlatformOwner]},async request=>app.db.transaction(async client=>{
    ...
    await writePlatformAudit(client,{actorId:request.platformAuth.actorId,action:'icon.library.delete_draft',targetType:'platform_icon',targetId:row.id,metadata:{key:row.key,source_type:row.source_type},requestIp:request.ip,requestId:request.id});
