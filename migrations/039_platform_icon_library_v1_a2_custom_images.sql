...
ALTER TABLE platform_icons ADD COLUMN category text;
ALTER TABLE platform_icons ADD CONSTRAINT platform_icons_category_check CHECK (category IS NULL OR char_length(category) BETWEEN 1 AND 80);
...
CREATE TABLE platform_icon_assets (
  ...
  icon_id uuid NOT NULL REFERENCES platform_icons(id) ON DELETE CASCADE,
  variant text NOT NULL DEFAULT 'DEFAULT' CHECK (variant IN ('DEFAULT','LIGHT','DARK')),
