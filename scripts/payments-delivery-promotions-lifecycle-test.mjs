...
try{
 ...
 const c=expect(await req({method:'POST',url:'/v1/customer/auth/register',headers:th,payload:{email:customerEmail,password,display_name:'Buyer'}}),201,'customer');const ch={authorization:`Bearer ${c.data.tokens.access_token}`};const customerId=c.data.customer.id;
 const other=expect(await req({method:'POST',url:'/v1/customer/auth/register',headers:th,payload:{email:otherEmail,password,display_name:'Other'}}),201,'other customer');const otherId=other.data.customer.id;
 ...
 expect(await req({method:'PATCH',url:`/v1/merchant/fulfillments/${f.id}`,headers:mh,payload:{status:'PREPARING'}}),200,'prepare');
 ...
 console.log('PASS fulfillment PREPARING → SHIPPED → DELIVERED tracking/status history lifecycle');
